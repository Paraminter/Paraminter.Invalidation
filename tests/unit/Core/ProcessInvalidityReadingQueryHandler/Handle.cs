...
public sealed class Handle
{
    ...
    [Fact]
    public async Task ValidQuery_ReadsInvalidity()
    {
        var expected = Mock.Of<object>();

        var fixture = FixtureFactory.Create<IQuery, object>();

        fixture.InvalidityReaderMock.Setup(static (handler) => handler.Handle(It.IsAny<IIsProcessInvalidatedQuery>(), It.IsAny<CancellationToken>())).Returns(Task.FromResult(expected));
